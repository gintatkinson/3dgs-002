...
void main() {
  ...
  calloc.free(config);
  print('Handle: $handle');
  assert(handle > 0, 'Initialize failed');
  ...
  assert(latDelta < 0.001, 'lat precision: $latDelta');
  assert(lngDelta < 0.001, 'lng precision: $lngDelta');
  assert(altDelta < 0.1, 'alt precision: $altDelta');
