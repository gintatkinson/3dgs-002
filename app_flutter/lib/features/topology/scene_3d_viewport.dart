    ...
    if (key == LogicalKeyboardKey.arrowLeft) {
      setState(() {
        _camera = VirtualCamera(
          ...
          longitude: _wrapLng(_camera.longitude - 5),
    ...
    if (key == LogicalKeyboardKey.arrowUp) {
      setState(() {
        _camera = VirtualCamera(
          ...
          altitude: _camera.altitude,
          heading: _camera.heading,
          pitch: (_camera.pitch + 5).clamp(-89.0, 89.0),
          roll: _camera.roll,
        );
      });
      return KeyEventResult.handled;
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        final zoomScale = 500.0 / _camera.altitude;
        return Stack(
          children: [
            ...
            Positioned.fill(
              child: Focus(
                ...
                child: GestureDetector(
                  ...
                  onPanUpdate: (details) {
                    setState(() {
                      final newLat = (_camera.latitude - details.delta.dy * 0.15).clamp(-90.0, 90.0);
                      final newLng = _wrapLng(_camera.longitude - details.delta.dx * 0.15);
                      _camera = VirtualCamera(
                        latitude: newLat,
                        longitude: newLng,
                        altitude: _camera.altitude,
                  ...
                  child: Listener(
                    onPointerSignal: (pointerSignal) {
                      if (pointerSignal is PointerScrollEvent) {
                        setState(() {
                          final newAlt = (_camera.altitude + pointerSignal.scrollDelta.dy * 0.5).clamp(100.0, 40000000.0);
                          _camera = VirtualCamera(
                            latitude: _camera.latitude,
                            longitude: _camera.longitude,
                            altitude: newAlt,
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          'Altitude: ${_camera.altitude} meters',
                          style: const TextStyle(
                            color: Color(0xFFE0E0E0),
                            fontFamily: 'monospace',
                            fontSize: 11,
                        ...
                        Text(
                          'Pitch/Yaw/Roll: ${_camera.pitch} / ${_camera.heading} / ${_camera.roll}',
                          style: const TextStyle(
                            color: Color(0xFFE0E0E0),
                            fontFamily: 'monospace',
                            fontSize: 11,
  ...
  ProjectedPoint _project(double lat, double lng, double sphereRadius, Offset center, double rotationY, double tilt) {
    ...
    if (engine != null && engine.isReady) {
      ...
      if (ecef != null) {
        ...
        return ProjectedPoint(Offset(center.dx + xFinal, center.dy - yFinal), zFinal);
      }
    }
    ...
    if (astronomicalBody != 'Proxima Centauri') {
      ...
      for (final (latMax, latMin, color) in bands) {
        ...
        for (int s = 0; s <= steps; s++) {
          ...
          pts.add(_project(latMin, lng, sphereRadius * 1.002, center, rotationAngle, tilt));
        }
        for (int s = steps; s >= 0; s--) {
          final double lng = s * (2 * math.pi / steps);
          pts.add(_project(latMax, lng, sphereRadius * 1.002, center, rotationAngle, tilt));
        }
